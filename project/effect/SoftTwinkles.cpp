...
SoftTwinkles::SoftTwinkles(struct CRGB *pLeds) {
	leds = pLeds;
}

char* SoftTwinkles::name() {
	return "SoftTwinkles";
}

void SoftTwinkles::show() {
...
//
void SoftTwinkles::twinkle() {
	for( int i = 0; i < NUM_LEDS; i++) {
		if( !leds[i]) continue; // skip black pixels
		if( leds[i].r & 1) { // is red odd?
			leds[i] -= twinkleColor; // darken if red is odd
			} else {
			leds[i] += twinkleColor; // brighten if red is even
		}
	}
